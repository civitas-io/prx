...
- **Benchmark CI root resolution** — CLI `root` argument now overrides dataset
  `root` field when explicitly provided. Fixes all 8 benchmark jobs failing in CI
  because datasets had local `benchmarks/checkouts/...` paths baked in.
- Removed hardcoded local root paths from all 8 benchmark dataset JSON files.
